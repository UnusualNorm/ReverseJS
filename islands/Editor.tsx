...
import { getOrCreateModel, noop } from "utils/monaco.ts";
import type { EditorProps, Monaco } from "types/monaco.ts";
import MonacoContainer from "components/MonacoContainer.tsx";
...
function Editor({
  ...
  useEffect(() => {
    if (isEditorReady) {
      const changeMarkersListener = monacoRef.current!.editor
        .onDidChangeMarkers((uris) => {
          const editorUri = editorRef.current!.getModel()?.uri;

          if (editorUri) {
            const currentEditorHasMarkerChanges = uris.find((uri) =>
